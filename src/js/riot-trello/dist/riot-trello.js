var RiotTrello = (function () {
	...
	class List extends HTMLElement {
		constructor(options = {}) {
			...
			this.shadowRoot.innerHTML = `<style>.card{margin-bottom:10px;position:relative}.card-label{width:10px;height:50%;position:absolute;left:30px;top:25%;border-radius:50%}.list-title{color:white;background-color:#205c7e;border-radius:50px;height:40px;position:relative}</style>`;
	...
	customElements.define("riot-trello-list", List);
	...
	class Board extends HTMLElement {
		constructor(options = {}) {
			...
			this.attachShadow({ mode: 'open' });
			this.shadowRoot.innerHTML = `<style>h1{color:black}.board{background-color:white;border:1px solid #205c7e;border-radius:10px;color:black}.error{color:red}</style>`;
